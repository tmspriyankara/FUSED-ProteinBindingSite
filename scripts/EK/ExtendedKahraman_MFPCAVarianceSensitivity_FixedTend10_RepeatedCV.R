# =====================================================================
# Extended Kahraman | MFPCA variance sensitivity at fixed t_end = 10 A
#   - Fixed t_start = 4.8 A and fixed t_end = 10.0 A.
#   - Compares MFPC scores retaining 95% vs 98% training variance.
#   - Repeated stratified 5-fold CV: 20 repeats x 5 folds = 100 pairs.
#   - Same folds and same train-only PCA are used for both variance targets.
#   - Paired sign-flip permutation tests assess whether 98% changes accuracy.
# =====================================================================
...
MAX_PC <- 20
VAR_TARGETS <- c(`95pct` = 0.95, `98pct` = 0.98)
...
build_mfpca_scores_for_targets <- function(smoothed, train_idx, test_idx) {
  ...
  out <- list()
  for (nm in names(VAR_TARGETS)) {
    target <- VAR_TARGETS[[nm]]
    k_use <- which(cum_var >= target)[1]
    ...
    out[[nm]] <- list(Ztr = Ztr, Zte = Zte, k_use = k_use, var_target = target)
  }
...
paired_test_summary <- function(df_model) {
  d <- df_model$accuracy_98 - df_model$accuracy_95
  obs <- mean(d)
  ...
  data.frame(
    ...
    t_end = t_end_fixed,
    accuracy_95_mean = mean(df_model$accuracy_95),
    accuracy_95_sd = sd(df_model$accuracy_95),
    k_95_mean = mean(df_model$k_95),
    k_95_sd = sd(df_model$k_95),
    accuracy_98_mean = mean(df_model$accuracy_98),
    accuracy_98_sd = sd(df_model$accuracy_98),
    k_98_mean = mean(df_model$k_98),
    k_98_sd = sd(df_model$k_98),
    mean_paired_difference_98_minus_95 = obs,
...
# =====================================================================
# load cache and build fixed-threshold representation
# =====================================================================
...
cat("Comparing MFPCA variance targets:", paste(VAR_TARGETS, collapse = " vs "), "\n")
print(table(labels))
...
for (rep_id in seq_len(N_REPEATS)) {
  ...
  folds <- make_strat_folds(labels, K_FOLDS, seed = 14000 + rep_id)
  ...
  for (fold_id in seq_len(K_FOLDS)) {
    ...
    scores <- build_mfpca_scores_for_targets(smoothed, train_idx, test_idx)
    s95 <- scores$`95pct`
    s98 <- scores$`98pct`

    seed_use <- 15000 + 100 * rep_id + fold_id
    rf_95 <- rf_predict_accuracy(s95$Ztr, y_tr, s95$Zte, y_te, seed = seed_use)
    rf_98 <- rf_predict_accuracy(s98$Ztr, y_tr, s98$Zte, y_te, seed = seed_use)

    lasso_95 <- lasso_predict_accuracy(s95$Ztr, y_tr, s95$Zte, y_te)
    lasso_98 <- lasso_predict_accuracy(s98$Ztr, y_tr, s98$Zte, y_te)

    mah_95 <- mah_predict_accuracy(s95$Ztr, y_tr, s95$Zte, y_te)
    mah_98 <- mah_predict_accuracy(s98$Ztr, y_tr, s98$Zte, y_te)
    ...
    fold_results[[length(fold_results) + 1L]] <- data.frame(
      ...
      t_end = t_end_fixed,
      accuracy_95 = c(rf_95, lasso_95, mah_95),
      accuracy_98 = c(rf_98, lasso_98, mah_98),
      paired_difference_98_minus_95 = c(rf_98 - rf_95, lasso_98 - lasso_95, mah_98 - mah_95),
      k_95 = s95$k_use,
      k_98 = s98$k_use,
      stringsAsFactors = FALSE
    )
    ...
    cat(sprintf(
      "Repeat %d | Fold %d | k95=%d k98=%d | RF %.3f -> %.3f | Lasso %.3f -> %.3f | Mah %.3f -> %.3f\n",
      rep_id, fold_id, s95$k_use, s98$k_use,
      rf_95, rf_98, lasso_95, lasso_98, mah_95, mah_98
    ))

    rm(scores, s95, s98); invisible(gc())
  }
}
...
write.csv(
  fold_df,
  file.path(out_dir, "EK_mfpcaVarianceSensitivity_fixedTend10_repeated20x5_fold_results.csv"),
  row.names = FALSE
)
write.csv(
  summary_df,
  file.path(out_dir, "EK_mfpcaVarianceSensitivity_fixedTend10_repeated20x5_summary.csv"),
  row.names = FALSE
)

cat("\n===== EK MFPCA variance sensitivity at fixed t_end = 10 A =====\n")
print(summary_df)
