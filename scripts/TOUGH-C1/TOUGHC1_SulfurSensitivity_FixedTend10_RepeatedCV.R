# =====================================================================
# TOUGH-C1 | Sulfur sensitivity analysis at fixed t_end = 10 A
#   - Fixed t_start = d_min_sel and fixed t_end = 10.0 A.
...
rr_cache_file <- file.path(cache_dir, "TOUGH_C1_LBS_ligand_ILR_CDPA_to20A.rds")
sulfur_cache_file <- file.path(cache_dir, "TOUGH_C1_sulfur_present_at10A.rds")

if (!file.exists(rr_cache_file)) {
  stop("Missing TOUGH-C1 descriptor cache: ", rr_cache_file)
}
...
# =====================================================================
# helpers: files and sulfur indicator
# =====================================================================
resolve_existing_path <- function(path_value) {
  candidates <- unique(c(
    path_value,
    file.path("data", "TOUGH-C1", path_value),
...
read_pdb_like_minimal <- function(path) {
  ...
  elem <- toupper(substr(elem, 1, 1))
  keep <- elem != "H" & is.finite(x) & is.finite(y) & is.finite(z)

  data.frame(
    rec = rec[keep], resn = resn[keep], chain = chain[keep],
...
build_sulfur_at_10_cache <- function(obj, save_path) {
  pairs_df <- obj$pairs_df
  sulfur_present <- rep(FALSE, nrow(pairs_df))
  sulfur_count <- rep(NA_integer_, nrow(pairs_df))

  cat("Building TOUGH-C1 sulfur-at-10A cache...\n")
  for (i in seq_len(nrow(pairs_df))) {
    p_df <- read_pdb_like_minimal(resolve_existing_path(pairs_df$p_file[i]))
    l_df <- read_pdb_like_minimal(resolve_existing_path(pairs_df$l_file[i]))

    if (!is.null(p_df) && !is.null(l_df) && nrow(p_df) > 0 && nrow(l_df) > 0) {
      p_coords <- as.matrix(p_df[, c("x", "y", "z")])
      l_coords <- as.matrix(l_df[, c("x", "y", "z")])
      dmin <- min_dist_to_ligand(p_coords, l_coords)
      site <- p_df[dmin <= t_end_fixed, , drop = FALSE]
      sulfur_count[i] <- sum(site$elem == "S")
      sulfur_present[i] <- sulfur_count[i] > 0
    }

    if (i %% 250 == 0) cat("sulfur processed", i, "of", nrow(pairs_df), "\n")
  }

  out <- list(
    sulfur_present_at_10A = sulfur_present,
    sulfur_count_at_10A = sulfur_count,
    t_end = t_end_fixed,
    pairs_df = pairs_df
  )
  saveRDS(out, save_path)
  cat("Saved sulfur cache:", save_path, "\n")
  out
}

# =====================================================================
# helpers: CV and classifiers
# =====================================================================
...
smooth_all_channels <- function(ilr_arr_task, cov_arr_task, idx_use, thr_full) {
...
run_task <- function(obj, sulfur_at_10, cls_pos, cls_neg, suffix) {
  idx_task <- which(obj$labels %in% c(cls_pos, cls_neg))
  y_task <- droplevels(obj$labels[idx_task])
  ilr_arr_task <- obj$ilr_arr[idx_task, , , drop = FALSE]
  cov_arr_task <- obj$cov_arr[idx_task, , , drop = FALSE]
  sulfur_task <- sulfur_at_10[idx_task]

  t_start <- obj$d_min_sel
  idx_use <- which(obj$thr_full >= t_start & obj$thr_full <= t_end_fixed)

  cat("\n============================================================\n")
  cat("TOUGH-C1 sulfur sensitivity task:", suffix, "\n")
  cat("Fixed interval:", t_start, "to", t_end_fixed, "Angstrom\n")
  cat("Samples:", length(y_task), "\n")
  cat("Sulfur present at 10 A:", sum(sulfur_task), "of", length(sulfur_task), "\n")
  print(table(y_task))

  cat("Smoothing fixed-threshold functional descriptors...\n")
  smoothed <- smooth_all_channels(ilr_arr_task, cov_arr_task, idx_use, obj$thr_full)

  fold_results <- list()

  for (rep_id in seq_len(N_REPEATS)) {
    cat("\nTask", suffix, "| Repeat", rep_id, "of", N_REPEATS, "\n")
    folds <- make_strat_folds(y_task, K_FOLDS, seed = 7000 + 100 * match(cls_pos, c("NUC", "HEME")) + rep_id)

    for (fold_id in seq_len(K_FOLDS)) {
      test_idx <- folds[[fold_id]]
      train_idx <- setdiff(seq_along(y_task), test_idx)
      y_tr <- droplevels(y_task[train_idx])
      y_te <- factor(y_task[test_idx], levels = levels(y_tr))

      scores <- build_mfpca_scores(smoothed, train_idx, test_idx)
      scores_s <- add_sulfur_indicator(
        scores,
        sulfur_train = sulfur_task[train_idx],
        sulfur_test = sulfur_task[test_idx]
      )

      seed_use <- 8000 + 1000 * match(cls_pos, c("NUC", "HEME")) + 100 * rep_id + fold_id
      rf_base <- rf_predict_accuracy(scores$Ztr, y_tr, scores$Zte, y_te, seed = seed_use)
      rf_s <- rf_predict_accuracy(scores_s$Ztr, y_tr, scores_s$Zte, y_te, seed = seed_use)
      ...
      fold_results[[length(fold_results) + 1L]] <- data.frame(
        ...
        repeat_id = rep_id,
        fold_id = fold_id,
        model = c("RF", "BinomialLasso", "Mahalanobis"),
        t_end = t_end_fixed,
      ...
      cat(sprintf(
        "Task %s | Repeat %d | Fold %d | RF %.3f -> %.3f | Lasso %.3f -> %.3f | Mah %.3f -> %.3f\n",
        suffix, rep_id, fold_id, rf_base, rf_s, lasso_base, lasso_s, mah_base, mah_s
      ))
  ...
  summary_df <- summary_df[match(c("RF", "BinomialLasso", "Mahalanobis"), summary_df$model), ]
  rownames(summary_df) <- NULL

  list(fold = fold_df, summary = summary_df)
}

# =====================================================================
# load/build caches and run tasks
# =====================================================================
obj <- readRDS(rr_cache_file)
obj$thr_full <- as.numeric(obj$thr_full)
obj$labels <- droplevels(obj$labels)

if (file.exists(sulfur_cache_file)) {
  sulfur_raw <- readRDS(sulfur_cache_file)
  sulfur_at_10 <- sulfur_raw$sulfur_present_at_10A
  cat("Loaded sulfur cache:", sulfur_cache_file, "\n")
} else {
  sulfur_raw <- build_sulfur_at_10_cache(obj, sulfur_cache_file)
  sulfur_at_10 <- sulfur_raw$sulfur_present_at_10A
}

if (length(sulfur_at_10) != length(obj$labels)) {
  stop("Sulfur indicator length does not match TOUGH-C1 cache labels.")
}

res_nuc <- run_task(obj, sulfur_at_10, "NUC", "CONTROL", "NUC_vs_CONTROL")
res_heme <- run_task(obj, sulfur_at_10, "HEME", "CONTROL", "HEME_vs_CONTROL")

fold_all <- rbind(res_nuc$fold, res_heme$fold)
summary_all <- rbind(res_nuc$summary, res_heme$summary)

write.csv(
  fold_all,
  file.path(out_dir, "TOUGHC1_sulfurSensitivity_fixedTend10_repeated20x5_fold_results.csv"),
  row.names = FALSE
)
write.csv(
  summary_all,
  file.path(out_dir, "TOUGHC1_sulfurSensitivity_fixedTend10_repeated20x5_summary.csv"),
  row.names = FALSE
)

cat("\n===== TOUGH-C1 sulfur sensitivity at fixed t_end = 10 A =====\n")
print(summary_all)
