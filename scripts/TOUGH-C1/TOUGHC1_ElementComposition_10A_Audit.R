# =====================================================================
# TOUGH-C1 | Element composition audit at 10 Angstrom
#   - Counts elements in protein binding-site atoms only.
#   - Ligand atoms are used only to define the binding site, not counted.
# =====================================================================

rm(list = ls()); invisible(gc())

# ------------------- paths -------------------
cache_file <- file.path("data", "cache", "TOUGH_C1_LBS_ligand_ILR_CDPA_to20A.rds")
out_dir <- file.path("results", "TOUGH-C1")
dir.create(out_dir, recursive = TRUE, showWarnings = FALSE)

if (!file.exists(cache_file)) {
  stop("Descriptor cache not found. Run the TOUGH-C1 nested-CV script first to create: ", cache_file)
}
...
element_from_pdbqt_type <- function(atom_name, atom_type) {
  atom_type <- toupper(trimws(atom_type))
  atom_name <- toupper(trimws(atom_name))

  if (atom_type %in% c("A", "C")) return("C")
  if (atom_type %in% c("N", "NA", "NS")) return("N")
  if (atom_type %in% c("O", "OA", "OS")) return("O")
  if (atom_type %in% c("S", "SA")) return("S")
  if (atom_type %in% c("P", "PA")) return("P")
  if (grepl("^H", atom_type)) return("H")

  elem <- gsub("[^A-Z]", "", atom_type)
  if (nchar(elem) > 0) return(substr(elem, 1, 1))

  elem <- gsub("[^A-Z]", "", atom_name)
  if (nchar(elem) > 0) return(substr(elem, 1, 1))
  NA_character_
}

read_protein_pdbqt_minimal <- function(path) {
  ...
  atom_type <- vapply(strsplit(trimws(ln), "\\s+"), function(parts) {
    parts[length(parts)]
  }, character(1))
  elem <- mapply(element_from_pdbqt_type, aname, atom_type, USE.NAMES = FALSE)

  keep <- rec == "ATOM  " & elem != "H" & is.finite(x) & is.finite(y) & is.finite(z)
  data.frame(
    rec = rec[keep],
...
read_ligand_coords_minimal <- function(path) {
  ln <- tryCatch(readLines(path, warn = FALSE), error = function(e) NULL)
  if (is.null(ln)) return(NULL)
  ln <- ln[grepl("^(ATOM  |HETATM)", ln)]
  if (length(ln) == 0) return(NULL)
  ...
  coords <- cbind(x[keep], y[keep], z[keep])
  if (nrow(coords) == 0) return(NULL)
  coords
}

get_site_atoms <- function(prot_df, lig_coords, dthr, min_atoms = 10) {
  if (is.null(prot_df) || is.null(lig_coords)) return(NULL)
  prot_coords <- as.matrix(prot_df[, c("x", "y", "z")])
  pick <- logical(nrow(prot_df))

  for (i in seq_len(nrow(lig_coords))) {
    dx <- prot_coords[, 1] - lig_coords[i, 1]
    dy <- prot_coords[, 2] - lig_coords[i, 2]
    dz <- prot_coords[, 3] - lig_coords[i, 3]
    pick <- pick | (sqrt(dx * dx + dy * dy + dz * dz) <= dthr)
  }

  site <- prot_df[pick, , drop = FALSE]
...
# =====================================================================
cat("Auditing TOUGH-C1 protein binding-site element composition at", dthr, "Angstrom\n")
...
stopifnot(length(labels) == nrow(pairs_df))

raw_count_records <- list()
site_total_records <- list()

for (i in seq_len(nrow(pairs_df))) {
  p_df <- read_protein_pdbqt_minimal(pairs_df$p_file[i])
  l_coords <- read_ligand_coords_minimal(pairs_df$l_file[i])
  site <- get_site_atoms(p_df, l_coords, dthr, min_atoms = 10)
  if (is.null(site)) next

  site_id <- pairs_df$p_base[i]
  ...
  if (i %% 250 == 0) cat("processed", i, "of", nrow(pairs_df), "\n")
}

if (length(raw_count_records) == 0 || length(site_total_records) == 0) {
  stop("No TOUGH-C1 sites were available for element auditing.")
}
...
write.csv(counts_df, file.path(out_dir, "TOUGHC1_element_counts_10A_by_site.csv"), row.names = FALSE)
...
write.csv(element_summary, file.path(out_dir, "TOUGHC1_element_summary_10A_all_elements.csv"), row.names = FALSE)
write.csv(class_element_summary, file.path(out_dir, "TOUGHC1_element_summary_10A_by_class.csv"), row.names = FALSE)
write.csv(site_non_con, file.path(out_dir, "TOUGHC1_non_CON_elements_10A_by_site.csv"), row.names = FALSE)
write.csv(site_non_con_summary, file.path(out_dir, "TOUGHC1_non_CON_summary_10A.csv"), row.names = FALSE)

cat("\n===== TOUGH-C1 elements observed or explicitly checked at 10 Angstrom =====\n")
...
cat("\n===== TOUGH-C1 non-C/O/N site-level summary at 10 Angstrom =====\n")
print(site_non_con_summary)

cat("\nSaved TOUGH-C1 10A element audit outputs under:", out_dir, "\n")
